...
Implementation of STDP — a temporally asymmetric form of Hebbian
learning observed in biological neural circuits. The key insight:
the precise timing of pre- and post-synaptic spikes determines
whether a synapse is strengthened (LTP) or weakened (LTD).

    Pre before Post  (Δt > 0)  →  LTP  (causal: pre caused post to fire)
    Post before Pre  (Δt < 0)  →  LTD  (anti-causal: reverse association)
...
class STDPNetwork:
    """
    A network of leaky integrate-and-fire neurons connected by STDP
    synapses.
